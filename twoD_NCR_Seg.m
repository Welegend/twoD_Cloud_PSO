...
for k = 1: w % 求第k段的云参数[Ex1, En1, He1, Ex2, En2, He2]
...
%% 实现1NN算法
[~, I] = max(Measure_Table, [], 2); % 每行相似度由大到小排序，I为m2维列向量
Predictlabel = trainlabel(I);              % 1NN算法的预测标签

%% error rate计算
correct_number = sum(Predictlabel == testlabel); % 正确个数
...
disp(['数据集：',Name, ' 分段数 ', num2str(w), ' error rate = ',num2str(error_rate)]);
